...
function Battery() {
    ...
    const getBatteryPercentage = () => {
        axios.get("http://blynk-cloud.com/qVJoK80A5WjpWihghgRjyqop6pP-8giy/get/V6")
            .then(res => {
    ...
    const getVoltageCurrent = () => {
        axios.get("http://blynk-cloud.com/qVJoK80A5WjpWihghgRjyqop6pP-8giy/get/V3")
            .then(res => {
                setBatteryAPIVoltage(parseFloat(res.data));
            })
        axios.get("http://blynk-cloud.com/qVJoK80A5WjpWihghgRjyqop6pP-8giy/get/V4")
            .then(res => {
                setBatteryAPICurrent(parseFloat(res.data));
            })
    ...
    const getBatteryPage = () => {
        axios.get("http://127.0.0.1:5000/dashboard/battery")
            .then(res => {
                // setData(res.data);
                console.log(res.data)
                setChargeCycle(res.data.chargeCycles);
    ...
    return (
        <div style={{ display: 'flex' }}>
                ...
                <div className="popup__bg">
                    <div className="popup__box">
                        ...
                        <div className="popup__form">
                            ...
                            <select className="battery__details__form__input" value={editBatteryType} onChange={(e) => setEditBatteryType(e.target.value)} >
                            ...
                            <select className="battery__details__form__input" value={editBatteryVoltage} onChange={(e) => setEditBatteryVoltage(e.target.value)} >
                                {
                                    (editBatteryType === 'Lithium Ion' || editBatteryType === 'Lithium Iron Phosphate') ? (
                                        batteryVoltage.liion.map((item, index) => (
                                            <option key={index}>{item}</option>
                                        ))
                                    )
                                        :
                                        (editBatteryType === 'Nickel Cadnium' || editBatteryType === 'Nickel Metal Hydride') ? (
                                            ...
                                            :
                                            (
                                                batteryVoltage.leadacid.map((item, index) => (
                                                    <option key={index}>{item}</option>
                                                ))
                                            )
                            ...
                            <select type="text" className="battery__details__form__input" value={editBatteryAmp} onChange={(e) => setEditBatteryAmp(e.target.value)}>
                        ...
                        <button className="battery__details__form__button popup__button green-p-bg" onClick={postBatteryPage} >
                            Save Changes
                        </button>
                    </div>
                </div>
            )}
            <div className="dashboard__container" style={{ width: "50%", height: "calc(100vh - 135px)" }}>
                ...
                <div className="battery__details">
                    <div className="battery__details__form">
                        ...
                        <select className="battery__details__form__input" value={chargeTill} onChange={(e) => chargeTill1(e)}>
                            <option>50%</option>
                            <option>55%</option>
                            <option>60%</option>
                            <option>65%</option>
                        ...
                        <select className="battery__details__form__input" value={dischargeTill} onChange={(e) => dischargeTill1(e)}>
                            <option>0%</option>
                            <option>5%</option>
                            <option>10%</option>
                            <option>15%</option>
                        ...
                        <input type="text" className="battery__details__form__input" value={chargeCycle} onChange={() => console.log("blah!!!")} />
                        <br />
                        <button className={buttonEdit ? "battery__details__form__button green-p-bg" : "battery__details__form__button"} onClick={postBatteryPage}>
                            Save Changes
                        </button>
                    </div>
                    <div className="battery__details__charge">
                        <div className="battery__details__charge__background">
                            <div className="battery__details__charge__background__inner" style={{ height: batteryHeight }}>
                                &nbsp;
                            </div>
                        </div>
                        <img src={BatteryImg} alt="battery" className="battery__details__charge__image" />
                    </div>
                </div>
            ...
            <div style={{ width: "48%", height: "calc(100vh - 135px)", marginLeft: "20px" }}>
                <div className="dashboard__container" style={{ height: "46.75%", overflow: "hidden", marginBottom: "20px", position: "relative" }}>
                    <div className="dashboard__container__title blue-p">
                        Battery Usage
                    </div>
                    <div className="hc1">
                ...
                <div className="dashboard__container" style={{ height: "50%", overflow: "hidden", position: "relative" }}>
                    <div className="dashboard__container__title blue-p">
                        Battery Log
                    </div>
                    {
                        issue === 1 ?
                            <img src={SuccessBattery} alt="battery" className="success__battery" />
                            :
                            (
                                issue === 2 ?
                                    <img src={WarningBattery2} alt="battery" className="success__battery" />
                                    :
                                    <img src={ErrorBattery} alt="battery" className="success__battery" />
                            )
                    }
                </div>
            </div>
